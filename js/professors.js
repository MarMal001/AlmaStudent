function generateProfessors(professors) {
    ...
    for (const professor of professors) {
        content += `<div class="container-fluid w-auto w-lg-55 m-2 p-0">
            <button class="btn btn-primary d-flex justify-content-between align-items-center text-start w-100 fw-bold" type="button" data-bs-toggle="collapse" data-bs-target="#${professor["professor"]}">
        ...
        <div id="${professor["professor"]}" class="collapse p-3 w-100 border border-primary border-2 rounded">
            <h6>Corsi:</h6>
            <ul class="d-flex flex-column align-items-start">`;
        for (const course of professor["courses"]) {
            content += `<li><a href="course.php?course=${course["code"]}" class="text-primary">${course["name"]}</a></li>`;
        }
        content += `</ul>
