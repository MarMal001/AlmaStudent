...
$templateParams["content"] = "rating_professor_content.php";
...
if(!isStudent() || $dbh->isStudentBanned($user)) {
    ...
    if (isset($_GET["course"]) && $dbh->courseExists($_GET["course"])) {
        if ($dbh->canRateCourse($user, $_GET["course"])[0]["existence"] && !$dbh->professorsOfCourseAlreadyRated($user, $_GET["course"])) {
            ...
            $message = "Non puoi recensire i docenti di questo corso in questo momento";
            $messageType = "danger";
            header("location: index.php");
        }
    } else {
